...
proc rank data=ipec.simulations groups=4 out=simulations;
var pred;
ranks quartile_pred;
run;
...
proc sort data=simulations; by quartile_pred; run;/*n=460213*/

/*True hospital size*/
proc sql;
create table hospitals as
select distinct anon_hospital_id as pseudo_hospital, count(anon_hospital_id) as hospital_size
...
/*randomly select 22 hospitals to be "less sick" and 22 to be "more sick"*/
proc surveyselect data=hospitals out=hospitalsb seed=&iteration. n=44;
id pseudo_hospital;
run;

/*determine which half are more sick and which are less sick*/
data hospitalsb;
set hospitalsb;
...
data hospitalsb;
set hospitalsb;
if _n_<= 22 then casemix="less_sick";
else casemix="more_sick";
drop random;
run;

proc sort data=hospitals; by pseudo_hospital; run;
proc sort data=hospitalsb; by pseudo_hospital; run;

/*join the less_sick/more_sick assignment to the hospitals dataset*/
data hospitals&iteration.;
merge hospitals hospitalsb;
by pseudo_hospital;
run;
...
/*Determine the total number of patients in the less sick hospitals*/
...
/*Create a macro variable for the sample size needed for less sick and more sick hosps*/
data casemix_sizes;
set casemix_sizes;
...
/*patients assigned to less_sick hospitals.
Randomly sample the number of patients needed for the less_sick hospitals
where 40% are in the Q0, 30% in Q1, 20% in Q2 and 10% in Q3*/
proc sort data=simulations; by quartile_pred; run;

proc surveyselect data=simulations out=less_sick method=srs sampsize=&sampsizey.
seed=&iteration.;
id id id2;
strata quartile_pred/ alloc=(0.4 0.3 0.2 0.1);
run;

/*remove patients assigned to less_sick hospitals from the patient pool*/
proc sql;
create table sample2 as
select a.*
from simulations a
left join less_sick b
on a.id=b.id and a.id2=b.id2
where b.id is null and b.id2 is null;
quit;

proc sort data=sample2; by quartile_pred; run;

/*patients assigned to more_sick hospitals
Randomly sample the number of patients needed for the more_sick hospitals
where 10% are in the Q0, 20% in Q1, 30% in Q2 and 40% in Q3**/
proc surveyselect data=sample2 out=more_sick method=srs sampsize=&sampsizeo.
seed=&iteration.;
id id id2 ;
strata quartile_pred/ alloc=(0.1 0.2 0.3 0.4);
run;

/*remove patients assigned to more_sick hospitals from the patient pool*/
proc sql;
create table median as
select a.*
from sample2 a
left join more_sick b
on a.id=b.id and a.id2=b.id2
where b.id is null and b.id2 is null;
quit;

/*Randomly allocate patients assigned to less_sick hospitals to pseudo hospitals*/
proc surveyselect data=less_sick out=less_sick2
seed=&iteration. groups=22;
id id id2 ;
run;

proc surveyselect data=more_sick out=more_sick2
seed=&iteration. groups=22;
id id id2;
run;

proc surveyselect data=median out=median2
seed=&iteration. groups=67;
id id id2 ;
run;

proc sql;
create table less_sick3 as
select a.id, a.id2, b.pseudo_hospital, b.casemix
from less_sick2 a
left join hospitals&iteration. b
on a.groupid=b.hosp_id
where b.casemix="less_sick";
quit;

proc sql;
create table more_sick3 as
select a.id, a.id2, b.pseudo_hospital, b.casemix
from more_sick2 a
left join hospitals&iteration. b
on a.groupid=b.hosp_id
where b.casemix="more_sick";
quit;
...
data randomized;
set less_sick3 more_sick3 median3
...
proc datasets;
delete less_sick more_sick median less_sick2 more_sick2 median2 less_sick3 more_sick3 median3
randomized hospitalsb sample2 casemix_sizes ;
run;

/*Randomly select 22 hospitals to be "under-performers" (i.e. In the bottom quintile)*/
%let seed=%eval(&iteration+10000);

proc surveyselect data=hospitals 
out=hospitalsb seed=&seed. n=44;
run;

/*determine which half are under-performers and which are better-performers*/
data hospitalsb;
set hospitalsb;
random=ranuni(&seed.);
run;
...
data hospitalsb;
set hospitalsb;
if _n_<= 22 then hospital_effect="better";
else hospital_effect="under";
drop random;
run;
...
/*join the under/better assignment to the hospitals dataset*/
data hospitals;
merge hospitals hospitalsb;
...
data hospitals;
set hospitals;
if missing(hospital_effect) then hospital_effect="median";
run;

proc sort data=simulations; by pseudo_hospital; run;

data simulations;
merge simulations hospitals;
by pseudo_hospital;
run;

proc sort data=hospitals&iteration.; by pseudo_hospital; run;
proc sort data=hospitals; by pseudo_hospital; run;

data hospitals&iteration.;
merge hospitals&iteration. hospitals;
by pseudo_hospital;
run;
...
data simulations;
set simulations;
call streaminit(&iteration.); 
trials1=rand("Geometric", pred);
trials2=rand("Geometric", pred);
coin_flip=rand("Bernoulli", 0.5);

/*under- performers*/
if hospital_effect="under" then do;
/*if trials1=1 or trials2=1 then simulated_mort=1;*/
if trials1<=2 then simulated_mort=1;
else simulated_mort=0;
end;

/*median hospitals*/
if hospital_effect="median" then do; 
if trials1<=1 then simulated_mort=1;
else simulated_mort=0;
end;

/*better performers*/
if hospital_effect="better" then do;
if trials1<=1 and coin_flip=1 then simulated_mort=1;
else simulated_mort=0;
end;
run;
...
by pseudo_hospital;
model simulated_mort (event='1')=pred;
roc;
...
model simulated_mort=pred / noint link=logit dist=binomial solution cl ddfm=bw;
...
/*Hospital rank based on event rate. Identify the lowest quintile (hosps ranked 89-111)*/
data randomeffect&iteration.;
set randomeffect&iteration.;
	reg_hospital_rank+1;
	if reg_hospital_rank>=89 then reg_lower_quintile=1;
	if reg_hospital_rank<89 then reg_lower_quintile=0;
	if reg_hospital_rank>22 then reg_upper_quintile=0;
	if reg_hospital_rank<=22 then reg_upper_quintile=1;
run;
...
proc sort data=randomeffect&iteration.; by pseudo_hospital; run;
proc sort data=hospitals&iteration.; by pseudo_hospital; run;

data randomeffect&iteration.;
merge randomeffect&iteration. hospitals&iteration.;
by pseudo_hospital;
run;
...
select iteration, pseudo_hospital, casemix, hospital_effect, reg_hospital_rank, reg_lower_quintile, reg_upper_quintile,
...
proc surveyselect data=simulations noprint method = srs  sampsize = 240
   ...
   id id id2 anon_hospital_id site pseudo_hospital simulated_mort surgery
...
var simulated_mort pred glucose albval bili gfr bun na wbc hct pao2 surgery
	pco2 ph cx1  cx3 cx4 cx5 cx6 cx7 cx8 cx9 
	cx10 cx11 cx12 cx13 cx14 cx15 cx16 cx17 cx18 cx19 
	cx20 cx21 cx22 cx23 cx24 cx25 cx26 cx27 cx28 cx29 
	cx30 age sex white black hispanic opc nh ed  
...
read all var {simulated_mort pred glucose albval bili gfr bun na wbc hct pao2 surgery
	pco2 ph cx1  cx3 cx4 cx5 cx6 cx7 cx8 cx9 
	cx10 cx11 cx12 cx13 cx14 cx15 cx16 cx17 cx18 cx19 
	cx20 cx21 cx22 cx23 cx24 cx25 cx26 cx27 cx28 cx29 
	cx30 age sex white black hispanic opc nh ed 
...
select distinct(pseudo_hospital), mean(simulated_mort) as overalleventrate
...
select distinct(pseudo_hospital), sum(simulated_mort=1) as tm_event, 
count(simulated_mort) as n, mean(simulated_mort) as tm_eventrate
from allmatch&iteration. a
group by pseudo_hospital;
quit;

data allmatch&iteration.;
merge allmatch&iteration. tm_means overall&iteration. eventrate&iteration.;
by pseudo_hospital;
run;
...
proc sort data=tm_rates&iteration.; by pseudo_hospital; run;
proc sort data=hospitals&iteration.; by pseudo_hospital; run;

data tm_rates&iteration.;
merge tm_rates&iteration. hospitals&iteration.; 
by pseudo_hospital; run;

/*output the proportion of underperformers for each iteration*/
proc sql;
create table tm_rates as
select iteration, pseudo_hospital, casemix, hospital_effect, tm_hospital_rank, tm_lower_quintile, tm_upper_quintile,
...
pseudo_hosp_median_pred, pseudo_hosp_q1_pred, pseudo_hosp_q3_pred, _matchid as matchid
...
tm_mad tm_rates&iteration. tm_rates hospmd templatemd2 tm hospitalsb hospitals&iteration.; run;

data simulations;
set simulations;
drop pseudo_hospital roc lowerroc upperroc casemix hospital_effect;
run;

data hospitals;
set hospitals;
drop hospital_effect;
run;
