/*Scenario 2B: No true difference in hospital effects, no difference in hospital volume. 
Case mix varies by predicted mortality*/
%let scenario=2B;
%let n_hosp=111;
...
/*Assign patients to quartiles for predicted mortality*/
...
select iteration, pseudo_hospital, casemix, reg_hospital_rank, reg_lower_quintile, reg_upper_quintile,
...
proc datasets; 
delete rates randomeffect&iteration. mad reg_means reg_means2 auc; 
run;
...
class case;
psmodel case(treated='1')=
	pred glucose albval bili gfr bun na wbc hct pao2 surgery
	pco2 ph cx1  cx3 cx4 cx5 cx6 cx7 cx8 cx9 
	cx10 cx11 cx12 cx13 cx14 cx15 cx16 cx17 cx18 cx19 
	cx20 cx21 cx22 cx23 cx24 cx25 cx26 cx27 cx28 cx29 
	cx30 age sex white black hispanic opc nh ed 	
...
select iteration, pseudo_hospital, casemix, tm_hospital_rank, tm_lower_quintile, tm_upper_quintile,
...
tm_mad tm_rates&iteration. tm_rates hospmd templatemd2 tm hospitals&iteration.; run;

data simulations;
set simulations;
drop pseudo_hospital roc lowerroc upperroc casemix;
run;

proc sort data=simulations; by id; run;
%end;
%mend;
...
data tm.tm_sim_results&scenario.;
set tm.tm_sim_results&scenario.;
run;

options compress=yes;
data reg.reg_sim_results&scenario.;
set reg.reg_sim_results&scenario.;
run;
