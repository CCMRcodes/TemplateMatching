/*Scenario 7: No true difference in case-mix, hospital effects vary, hospital size varies*/
%let scenario=7;
%let n_hosp=111;
...
/*Randomly allocate patients to pseudo hospitals.
Hospital sizes are equivalent to true hospital sizes.*/
proc surveyselect data=ipec.simulations out=simulations noprint
seed=&iteration. groups=(&hospital_size.);
run;
...
out=hospitalsb seed=&iteration. n=44;
run;
...
data randomeffect&iteration.;
merge randomeffect&iteration. hospitals;
by pseudo_hospital; run;

/*output the proportion of underperformers for each iteration*/
proc sql;
create table rates as
select iteration, pseudo_hospital, hospital_effect, reg_hospital_rank, reg_lower_quintile, reg_upper_quintile,
...
proc datasets; 
delete auc mad randomeffect&iteration. rates reg_means reg_means2; 
run;

/*proc freq data=reg.reg_sim_rates&scenario.&sysdate.;*/
/*table reg_lower_quintile*hospital_effect/norow nocol nopercent;*/
/*run;*/
...
data allmatch&iteration.;
merge allmatch&iteration. tm_means eventrate&iteration. overall&iteration.;
by pseudo_hospital;
run;
...
data tm_rates&iteration.;
merge tm_rates&iteration. hospitals;
by pseudo_hospital;
run;
...
select iteration, pseudo_hospital, hospital_effect, tm_hospital_rank, tm_lower_quintile, tm_upper_quintile,
...
proc datasets;
delete tm_rates hospmd matchinfo md means population_means sim_template hospitalsb
sim_templatecases templatedata templatemd templatemdb templatedatahospital  tm tm_mad tm_means 
out_match&iteration.  overall&iteration. outmatch&iteration. tm_rates&iteration. allmatch&iteration. eventrate&iteration.;
run;
