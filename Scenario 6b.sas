/*Scenario 6B: No true difference in hospital effects, but differences in case-mix.
...
data simulations;
set ipec.simulations;
run;

/*Assign patients to quartiles for illness severity*/
proc rank data=simulations groups=4 out=quartiles;
var pred;
ranks quartile_pred;
run;
...
/*join the less sick/more sick assignment to the hospitals dataset*/
proc sql;
...
/*patients assigned to less sick hospitals.
Randomly sample the number of patients needed for the less sick hospitals
where 40% are in the Q0, 30% in Q1, 20% in Q2 and 10% in Q3*/
proc sort data=simulations; by quartile_pred; run;

proc surveyselect data=simulations out=less method=srs sampsize=&sampsizey.
seed=&iteration.;
id id id2;
strata quartile_pred/ alloc=(0.4 0.3 0.2 0.1);
run;

/*remove patients assigned to less sick hospitals from the patient pool*/
proc sql;
...
/*patients assigned to more sick hospitals
Randomly sample the number of patients needed for the more sick hospitals
where 10% are in the Q0, 20% in Q1, 30% in Q2 and 40% in Q3**/
proc surveyselect data=sample2 out=more method=srs sampsize=&sampsizeo.
seed=&iteration.;
id id id2 ;
strata quartile_pred/ alloc=(0.1 0.2 0.3 0.4);
run;

/*remove patients assigned to more sick hospitals from the patient pool*/
proc sql;
...
/*Assign "less sick hospital" patients to one of the 22 less sick hospitals*/
proc sql noprint;
select hospital_size into :less_hosps separated by ' '
from hospitals&iteration.
where casemix="less_sick";
quit;

/*Assign "more sick hospital" patients to one of the 22 more sick hospitals*/
proc sql noprint;
select hospital_size into :more_hosps separated by ' '
from hospitals&iteration.
where casemix="more_sick";
quit;
...
/*Randomly allocate patients assigned to less sick hospitals to pseudo hospitals*/
proc surveyselect data=less out=less2
seed=&iteration. groups=(&less_hosps.);
id id id2 ;
run;

proc surveyselect data=more out=more2
seed=&iteration. groups=(&more_hosps.);
id id id2;
run;
...
proc sql;
create table less3 as
select a.id, a.id2, b.hospital as pseudo_hospital, b.casemix
from less2 a
...
proc datasets;
delete less more median less2 more2 median2 less3 more3 median3
